...
public class PostUploadRequest extends Request<String> {
    private final static String TAG = "PostUploadRequest";
    /**
     * 正确数据的时候回掉用
     */
    ...
    public PostUploadRequest(String url, List<FormImage> listItem, ResponseListener listener) {
        ...
        mListItem = listItem ;
        setRetryPolicy(new DefaultRetryPolicy(25000,DefaultRetryPolicy.DEFAULT_MAX_RETRIES,DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
    }
    ...
    protected Response<String> parseNetworkResponse(NetworkResponse response) {
        try {
            String mString =
                    new String(response.data, HttpHeaderParser.parseCharset(response.headers,DEFAULT_PARAMS_ENCODING));
